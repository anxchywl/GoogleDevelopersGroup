...
export function metadata(locale: Locale): Metadata {
  ...
  return {
    ...
    ...(origin ? { metadataBase: new URL(origin), alternates: { canonical: localePath(locale), languages: { en: "/", kk: "/kk/", ru: "/ru/", "x-default": "/" } } } : {}),
    robots: origin ? { index: true, follow: true } : { index: false, follow: false },
    icons: { icon: "/icon.svg", apple: "/apple-icon.png" },
    openGraph: { title: event.name, description: c.heroLead, locale: { en: "en_US", kk: "kk_KZ", ru: "ru_RU" }[locale], type: "website", images: [{ url: origin ? new URL("/og.png", origin).href : "/og.png", width: 1200, height: 630, alt: `${event.name} · ${event.organizer}` }] },
