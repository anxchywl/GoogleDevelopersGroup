...
import { event } from "../src/content/event.ts";
...
const card = h("div", { style: { display:"flex", flexDirection:"column", justifyContent:"space-between", width:"100%", height:"100%", padding:"58px 72px", background:"#fafafa", color:"#1c1f23", fontFamily:"Noto Sans" } },
  ...
  h("div", { style:{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:48 } },
    h("div", { style:{ display:"flex", flexDirection:"column", gap:18 } },
      h("div", { style:{ fontSize:88, fontWeight:600, letterSpacing:-5 } }, event.name),
      h("div", { style:{ fontSize:30, color:"#174ea6" } }, event.tagline)),
...
const response = new ImageResponse(card, { width:1200, height:630, fonts:[{ name:"Noto Sans", data:font, weight:600, style:"normal" }] });
